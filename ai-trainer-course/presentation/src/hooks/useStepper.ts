 ...
 */
const STORAGE_KEY = "presentation-v4";
...
const [cursor, setCursor] = useState<Cursor>(() => {
    ...
    // URL param takes priority over localStorage (supports ExamMaster chapter jump)
    try {
      const initChapter = window.sessionStorage.getItem("presentation-init-chapter");
      if (initChapter !== null) {
        const parsed = JSON.parse(initChapter);
        window.sessionStorage.removeItem("presentation-init-chapter");
        return sanitize(parsed, chapters);
      }
    } catch {
      /* ignore */
    }

...
**原理**：章节目录 `src/chapters/` 以 `{三位数编号}-{前缀}-{描述}` 命名（如 `483-f1-scifi-bubble`）。章节在课程列表中的**排序由目录名字母顺序决定**，而非 `course.json`。**不同课程如果在同一三位数编号下有目录，排序后会交叉排列**，导致播放顺序错乱。

**检查方法**：在 `src/chapters/` 目录下执行：
```bash
ls {目标编号}-*/     # 检查该编号是否已被占用
# 例：655-*/ 存在两个目录 → 冲突
```
...
**冲突解决**：
1. 查询该编号区间（如 655-700）是否全部被占用
2. 如果已满，方案 A：新增章节使用 **大于当前最大编号**的数字，追加到课程末尾（通过 `course.json` 将其归入正确课程段）
3. 方案 B（需要时）：将区间靠后的现有目录重新编号到更大数字，腾出编号空隙给新章节
4. **音频目录不受影响**：音频存在 `public/audio/<章节ID>/`（按 ID 非编号），重编号后无需移动或重合成音频

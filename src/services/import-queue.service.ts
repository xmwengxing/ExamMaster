...
export interface ImportTaskProgress {
  stage: 'parsing' | 'validating' | 'inserting' | 'completed';
  processed: number;
  total: number;
  percentage: number;
}
...
export class ImportQueueService {
   ...
   * 添加导入任务到队列
   */
  async addImportTask(data: ImportTaskData): Promise<string> {
    // 在数据库中创建任务记录
    await db.query(
   ...
   * 获取任务结果
   */
  async getTaskResult(taskId: string): Promise<ImportTaskResult | null> {
    const result = await db.query(
   ...
   * 获取用户的所有任务
   */
  async getUserTasks(userId: string, limit: number = 50): Promise<any[]> {
    const result = await db.query(
...
importQueue.process(async (job) => {
  const { taskId, userId, filePath, fileName } = job.data as ImportTaskData;

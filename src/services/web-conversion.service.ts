...
export interface ParsedQuestion {
  content: string;
  type: 'SINGLE' | 'MULTIPLE' | 'JUDGE' | 'FILL_IN_BLANK' | 'SHORT_ANSWER';
  options?: string[];
...
export class WebConversionService {
   ...
   * 解析上传的文件
   */
  async parseFile(file: File): Promise<ParsedQuestions> {
   ...
   * 解析Excel文件
   */
  private async parseExcelFile(file: File): Promise<ParsedQuestions> {
   ...
   * 解析Excel行数据
   */
  private async parseExcelRow(row: any): Promise<ParsedQuestion | null> {
    // 获取题目内容
   ...
   * 解析Word文件
   */
  private async parseWordFile(file: File): Promise<ParsedQuestions> {
    const arrayBuffer = await file.arrayBuffer();
   ...
   * 从HTML中解析题目
   */
  private parseHTMLToQuestions(html: string): ParsedQuestion[] {
    const questions: ParsedQuestion[] = [];
   ...
   * 标准化题型
   */
  private normalizeQuestionType(typeStr: string): ParsedQuestion['type'] {
   ...
   * 解析选项
   */
  private parseOptions(optionsStr: string, type: ParsedQuestion['type']): string[] | undefined {
   ...
   * 解析答案
   */
  private parseAnswer(answerStr: string, type: ParsedQuestion['type']): string | string[] {
    if (!answerStr) return '';
   ...
   * 转换为系统JSON格式
   */
  convertToJSON(parsed: ParsedQuestions): QuestionBankJSON {
    return {
      metadata: {
        version: '2.0',
   ...
   * 验证转换结果
   */
  validateJSON(json: QuestionBankJSON): ValidationResult {

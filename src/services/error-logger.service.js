...
export class ErrorLoggerService {
  ...
  async logError(error, context) {
    const errorLog = {
      ...
      timestamp: new Date(),
      level: 'error',
      errorType: context.errorType || error.name || 'unknown',
   ...
   * 记录导入任务失败
   */
  async logImportTaskFailure(taskId, error, context) {
   ...
   * 记录图片处理失败
   */
  async logImageProcessingFailure(error, context) {
    return this.logError(error, {
      ...context,
      operation: context.operation,
   ...
   * 记录数据库操作失败
   */
  async logDatabaseOperationFailure(error, context) {
    return this.logError(error, {
      ...context,
      operation: context.operation,
   ...
   * 记录文件解析失败
   */
  async logFileParsingFailure(error, context) {
    return this.logError(error, {
      ...context,
